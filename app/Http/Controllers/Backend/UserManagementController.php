...
class UserManagementController extends Controller
{
     ...
     * Farm admin → only users from same farm
     */
    public function allUsers()
    {
        ...
        if ($currentUser->hasRole('super_admin')) {
            $users = User::with('detail', 'farm')->orderBy('created_at', 'desc')->get();
        } elseif ($currentUser->hasRole('farm_admin')) {
            $users = User::with('detail', 'farm')
                ->where('farm_id', $currentUser->farm_id)
    ...
    public function show(User $user)
    {
        ...
        if ($currentUser->hasRole('farm_admin') && $user->farm_id != $currentUser->farm_id) {
            abort(403, 'Unauthorized');
        }

        $user->load('farm', 'detail');
...
//         // Farm admin: see only users from their farm
//         else if ($currentUser->hasRole('farm_admin')) {
//             $users = User::with('detail', 'farm')
//                 ->where('farm_id', $currentUser->farm_id)
//                 ->orderBy('created_at', 'desc')
//                 ->get();
