...
class PanelRolesSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $farmAdminRole  = Role::updateOrCreate(['slug' => 'farm_admin'],  ['name' => 'Farm Admin']);
        ...
        for ($i = 1; $i <= 3; $i++) {
            // Pick random state and city for farm
            ...
            // Create Farm Admin
            $farmAdmins[$i] = User::create([
                'name' => "Farm Admin $i",
                'email' => "farmadmin$i@mail.com",
                'password' => Hash::make('12345678'),
                'farm_id' => $farms[$i]->id,
            ]);

            $farmAdmins[$i]->roles()->sync([$farmAdminRole->id]);
        }

        // -----------------------------
        // NORMAL USERS (3 per farm)
        // -----------------------------
        for ($i = 1; $i <= 3; $i++) {
            for ($u = 1; $u <= 3; $u++) {
                $user = User::create([
        ...
        foreach ($farms as $farm) {
            for ($a = 1; $a <= 5; $a++) {
                Animal::create([
                    'farm_id' => $farm->id,
                    'tag_number' => $farm->id . "-TAG" . $a,
                    'name' => "Animal $a - Farm $farm->id",
                    'type' => ['Cow','Goat','Sheep'][rand(0,2)],
        ...
        $this->command->info("✅ Seeded: SuperAdmin, All States, Cities, 3 Farms, 3 FarmAdmins, 9 Users, 15 Animals.");
    }
}
